<?php
require_once 'public/header.php';
?>

<div class="jumbotron">
    <div class="container">
        <h1>Sobre Nosotros</h1>
    </div>
</div>
<div class="containerRow row">
    <div class="row rowMapa">
        <div class="col-sm-12 text-center">
            <p>IF7103 Sistemas expertos para la administración</p>
            <p>Proyecto 1 Web</p>
        </div>
    </div>

    <div class="row rowMapa">
        <div class="col-sm-4 text-center"></div>
        <div class="col-sm-4 text-center">
            <p>En este sitio web mostramos diferentes actividades y atractivos turísticos. Además, tomando en cuenta algunos criterios que nos proporcionen para recomendar actividades o actractivos. <br> En otra sección se muestran reocmendaciones o tips que se deben tener en cuentan al viajar y visitar algunos de los atractivos y actividades existen. </p>
        </div>
        <div class="col-sm-4 text-center"></div>
    </div>

    <div class="row rowMapa">
        <div class="col-sm-4 text-center">
            <p class="tituloMapa">Heller Arguedas Quesada</p>
            <p>heller.arguedas@ucr.ac.cr</p>
            <p>Universidad de Costa Rica</p>
        </div>
        <div class="col-sm-4 text-center">
            <p class="tituloMapa">Steven Vargas Castro</p>
            <p>stevendlavc@gmail.com</p>
            <p>Universidad de Costa Rica</p>
        </div>
        <div class="col-sm-4 text-center">
            <p class="tituloMapa">Eyleen Quirós Hidalgo</p>
            <p>eyleenrqh@gmail.com</p>
            <p>Universidad de Costa Rica</p>
        </div>
    </div>

</div>

<?php
require_once 'public/footer.php';
?>
